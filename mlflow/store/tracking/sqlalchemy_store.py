...
class SqlAlchemyStore(SqlAlchemyGatewayStoreMixin, AbstractStore):
    ...
    def delete_experiment(self, experiment_id):
        with self.ManagedSessionMaker() as session:
            ...
            run_uuids = [run.run_uuid for run in runs]
            for run in runs:
                self._mark_run_deleted(session, run)
            session.add(experiment)
        self._delete_assessments_for_runs(run_uuids)
    ...
    def _delete_assessments_for_runs(self, run_uuids):
        """Best-effort cleanup of assessments produced by the given runs.

        Assessments generated by an evaluation run are linked to that run only
        through the ``mlflow.assessment.sourceRunId`` key inside the
        ``assessment_metadata`` JSON blob -- there is no foreign key or indexed
        column relating the two. This cleanup deliberately runs in its own
        transaction, after the primary run/experiment soft-delete has already
        been committed, so that a problem here never blocks the deletion itself.
        Failures are swallowed with a warning because the run is already marked
        deleted; leaving a few dangling assessments behind is far preferable to
        a 500 that makes runs undeletable. This guards against, for example, a
        tracking database whose schema predates the ``assessments`` table (a
        pending migration) or a slow scan over a very large assessments table.
        """
        if not run_uuids:
            return
        # The source run id lives inside a JSON text column, so matching is a
        # substring (LIKE) scan. Group runs together to avoid scanning the table
        # once per run when an entire experiment is deleted, but cap the batch
        # size so we never build a pathologically large OR statement (which can
        # exceed backend expression/parameter limits) for huge experiments.
        batch_size = 100
        try:
            with self.ManagedSessionMaker() as session:
                for i in range(0, len(run_uuids), batch_size):
                    batch = run_uuids[i : i + batch_size]
                    patterns = [
                        SqlAssessments.assessment_metadata.contains(
                            f'"{AssessmentMetadataKey.SOURCE_RUN_ID}": "{run_uuid}"'
                        )
                        for run_uuid in batch
                    ]
                    session.query(SqlAssessments).filter(or_(*patterns)).delete(
                        synchronize_session=False
                    )
        except Exception as e:
            _logger.warning(
                "Failed to delete assessments associated with deleted run(s) %s. "
                "The run(s) were still deleted successfully. Error: %s",
                run_uuids,
                e,
            )
    ...
    def delete_run(self, run_id):
        with self.ManagedSessionMaker() as session:
            run = self._get_run(run_uuid=run_id, session=session)
            self._mark_run_deleted(session, run)
        self._delete_assessments_for_runs([run_id])
